...
//   useEffect(() => {
    ...
    fabric.Image.fromURL('https://images.unsplash.com/photo-1681404141874-633aa2de7c76?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxlZGl0b3JpYWwtZmVlZHwyM3x8fGVufDB8fHx8&auto=format&fit=crop&w=200&q=60', function(img) {
        ...
        canvas1.current.add(img);
      });    
          
      
      canvas1.current.on('object:moving', function(e) {
        var obj = e.target;
        obj.setCoords(); // update the object's bounding box
        obj.set({ left: obj.left, top: obj.top }); // update the object's position
        canvas1.current.renderAll(); // render the canvas to show the updated object
      });      
  ...
  // canvas1.current.on("mouse:over",function (event) {  
